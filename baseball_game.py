...
def is_duplicated_number(three_digit):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    digit_set = set(three_digit)
    if len(three_digit) == len(digit_set):
    	result = False
    else:
    	result = True
...
def get_not_duplicated_three_digit_number():
    ...
    while is_duplicated_number(random_num):
    	random_num = str(get_random_number())

    result = int(random_num)
    # ==================================
    return result
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    # check strike
    ball_list = set(random_number)
    ball_user_input_number = list(user_input_number)
    for i in range(3):
    	if (user_input_number[i] == random_number[i]):
    		strike += 1
    		ball_list.remove(random_number[i])
    		ball_user_input_number.remove(user_input_number[i])
    
    # check ball
    ball = 0
    for digit in ball_list:
    	if digit in ball_user_input_number:
    		ball += 1
...
def main():
    ...
    while True:
        ...
        if len(user_input) == 1 and user_input[0] == '0':
            break
        while (not is_no(user_input)) and (not is_yes(user_input)):
            print("Wrong Input, Input again")
            user_input = input("You Win, one more(Y/N) ?")
            if len(user_input) == 1 and user_input[0] == '0':
                break
